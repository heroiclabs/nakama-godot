...
### Fixed
- Fixed code which causes Godot 3 to report memory leaks.
- Fixed links to Nakama documentation.
- Fixed naming of `HttpRequest` to match Godot's new naming.
- Fixed number of arguments passed to `HttpRequest.Request`.
- Fixed JSON `json_error` typo in `NakamaHTTPAdapter`.
- Removed some leftover Godot 3 code from master.

### Changed
- Godot 4 support is now on master. Godot 3 support can now be found on the `godot-3` branch.
- Changed return type from `int` to `TransferMode` in `_get_packet_mode` of the `NakamaMultiplayerPeer`.
